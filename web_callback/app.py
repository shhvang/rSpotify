...
async def spotify_callback(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # Check if temp_storage is initialized
        if temp_storage is None:
            logger.error('Temporary storage not initialized')
            return web.Response(
                text='<html><body><h1>Service Unavailable</h1>'
                     '<p>OAuth service is not ready. Please try again in a moment.</p></body></html>',
                content_type='text/html',
                status=503
            )
        ...
        if not telegram_id or not isinstance(telegram_id, (int, str)):
            logger.warning(f'Invalid or expired state parameter: {state} (got telegram_id: {telegram_id})')
            return web.Response(
        ...
        # CRITICAL: Use 'is None' to avoid pymongo Database.__bool__ NotImplementedError
        ...
        try:
            # Wrap synchronous pymongo operation in thread to avoid blocking event loop
            ...
            if result is None or not hasattr(result, 'inserted_id'):
                raise ValueError('Database insert returned invalid result')
                
        except PyMongoError as e:
            logger.error('PyMongo error storing auth code: %s', e, exc_info=True)
            return web.Response(
                text='<html><body><h1>Database Error</h1>'
                     '<p>Could not save your authorization code (MongoDB error).</p>'
                     '<p>Please return to Telegram and try /login again.</p></body></html>',
                content_type='text/html',
                status=500
            )
        except Exception as e:
            logger.error('Unexpected error storing auth code: %s', e, exc_info=True)
            return web.Response(
                text='<html><body><h1>Database Error</h1>'
                     '<p>Could not save your authorization code (unexpected error).</p>'
                     '<p>Please return to Telegram and try /login again.</p></body></html>',
                content_type='text/html',
                status=500
            )
        ...
        logger.error(f'[{error_id}] Error handling Spotify callback: {e}', exc_info=True)
        logger.error(f'[{error_id}] Error type: {type(e).__name__}')
        logger.error(f'[{error_id}] Request IP: {request.remote}')
        logger.error(f'[{error_id}] Request query: {dict(request.query)}')
        
        return web.Response(
            text=f'<html><body><h1>Internal Server Error</h1>'
                 f'<p>An error occurred processing your authorization.</p>'
                 f'<p>Error ID: <code>{error_id}</code></p>'
                 f'<p>Please return to Telegram and try /login again.</p></body></html>',
            content_type='text/html',
            status=500
        )
